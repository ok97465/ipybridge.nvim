...
local function setup_env()
  local fn = {
    ...
    tempname = function() return '/tmp/ipybridge_spec' end,
    writefile = function() return true end,
    fnamemodify = function(path, _) return path end,
    bufadd = function() return 1 end,
    bufload = function() end,
    jobstop = function() end,
  }

  local api = {
    nvim_replace_termcodes = function(_, _, _, _) return '' end,
  ...
  _G.vim = {
    ...
    loop = {
      os_uname = function() return { sysname = 'Test' } end,
  ...
  package.loaded['ipybridge.utils'] = {
    exec_file_stmt = function() return '' end,
    paste_block = function() return '' end,
    send_exec_block = function() return '' end,
    py_quote_single = function() return '' end,
    py_quote_double = function() return '' end,
    file_exists = function() return false end,
    selection_line_range = function() return nil end,
  }
  ...
  package.loaded['ipybridge.kernel'] = {
    ensure = function(_, cb) cb(true, '/tmp/connection.json') end,
    ensure_conn_file = function(_, cb) cb(true, '/tmp/connection.json') end,
    stop = function() end,
  }
  ...
  package.loaded['ipybridge.debug_vars'] = {
    digest_snapshot = function() return {} end,
    current_scope = function() return {} end,
    preview_payload = function() return nil end,
    ...
    new = function(state, opts)
      return setmetatable({
        ...
        __index = {
          ...
          after_helpers = function(_, cb) if cb then cb(true) end end,
          ...
          ensure_zmq = function(_, cb) if cb then cb(true) end end,
        },
      })
    end,
  }
