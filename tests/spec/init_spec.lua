...
it('open starts console and attaches breakpoint session', function()
  ...
  assert(ctx.kernel_calls and ctx.kernel_calls[1].python_cmd == 'python3', 'expected kernel.ensure to use default python')
  ...
  assert(env.IPYBRIDGE_BREAKPOINT_FILE == ctx.breakpoint_file or env.IPYBRIDGE_BREAKPOINT_FILE == '/tmp/breakpoints.json', 'breakpoint file not forwarded')
