						...
						for _, entry in ipairs(value) do
							if type(entry) == "table" then
								if entry.condition then
									table.insert(
										entry_parts,
										string.format('{"line":%d,"condition":"%s"}', entry.line or 0, entry.condition)
									)
								else
...
it("set_conditional_current_line stores condition and removes breakpoint on blank input", function()
	...
	assert(
		last_unplace and last_unplace.opts and last_unplace.opts.buffer == ctx.bufnr,
		"signs should be cleared on removal"
	)
...
it("toggle_current_line tracks lines and writes file", function()
	...
	assert(
		ctx.sign_place_calls[1] and ctx.sign_place_calls[1].name == "IpybridgeBreakpoint",
		"should use regular breakpoint sign"
	)
	assert(ctx.file_writes[#ctx.file_writes].lines[1]:find("%["), "file should be written with encoded payload")

	breakpoints.toggle_current_line()
	assert(
		ctx.last_encoded and next(ctx.last_encoded) == nil,
