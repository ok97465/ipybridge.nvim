...
def _viewer_html() -> str:
    ...
    try:
        ...
        _VIEWER_HTML_CACHE = f"<html><body><h1>Plot viewer unavailable</h1><p>{exc}</p></body></html>"
    return _VIEWER_HTML_CACHE
...
class PlotServer:
    ...
    def start(self) -> None:
        with self._lock:
            ...
            except OSError as exc:  # pragma: no cover - startup errors are environment-specific
                raise RuntimeError(f"plot server failed to bind: {exc}") from exc
            self._httpd = httpd
            self._port = httpd.server_address[1]
            thread = threading.Thread(target=httpd.serve_forever, name="ipybridge-plot-httpd", daemon=True)
            thread.start()
            self._thread = thread
    ...
    def set_backend_state(self, name: Optional[str], active: bool, required: Optional[str] = None) -> None:
    ...
    def remove_plot(self, plot_id: str) -> Optional[dict]:
        ...
        return {"ok": True, "removed": summary, "selected": selected.summary() if selected else None}
...
class _PlotServerController:
    ...
    def __init__(self, status_provider: Callable[[], dict], status_emitter: Callable[[Optional[dict]], dict]) -> None:
        self._status_provider = status_provider
    ...
    def set_backend_state(self, name: Optional[str], active: bool, required: str) -> None:
        if self._server:
...
class PlotRuntime:
    ...
    def __init__(self, status_callback: Optional[Callable[[dict], None]] = None) -> None:
        self._status_callback = status_callback
        self._required_backend = _PLOT_REQUIRED_BACKEND
        self._server_controller = _PlotServerController(lambda: self.status(), self._emit_status)
